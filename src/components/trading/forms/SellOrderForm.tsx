...
export function SellOrderForm({
  ...
  // Debug holdings data
  console.log('SellOrderForm - userHoldings:', userHoldings);
  console.log('SellOrderForm - totalQuantity:', userHoldings?.totalQuantity);
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <h4 className="text-sm font-medium text-blue-800 mb-2">Your Holdings</h4>
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <span className="text-blue-600">Total Shares:</span>
            <span className="font-medium text-blue-600  ml-1">{formatShares(totalHoldings)}</span>
          </div>
          <div>
            <span className="text-blue-600">Avg Cost:</span>
            <span className="font-medium text-blue-600 ml-1">{formatCurrency(avgCostBasis)}</span>
          </div>
          <div>
            <span className="text-blue-600">Current Value:</span>
            <span className="font-medium text-blue-600 ml-1">{formatCurrency(userHoldings?.currentValue || 0)}</span>
          </div>
          <div>
            <span className="text-blue-600">Unrealized P&L:</span>
            <span className={`font-medium text-blue-600 ml-1 ${
              (userHoldings?.unrealizedPnL || 0) >= 0 ? 'text-green-600' : 'text-red-600'
            }`}>
      ...
      {/* Quantity Input */}
      <div>
        <label className="text-sm font-medium text-gray-700 mb-3 block">Sale Amount</label>
