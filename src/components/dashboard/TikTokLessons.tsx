...
function TikTokLessonCard({ lesson, onClick }: TikTokLessonCardProps) {
  return (
    <div 
      className="relative aspect-[9/16] bg-neutral-900 rounded-2xl overflow-hidden cursor-pointer group transition-transform hover:scale-105"
      onClick={onClick}
    >
      {/* Background Image */}
      <Image
      ...
      <div className="absolute inset-0 p-4 flex flex-col justify-between">
        ...
        <div className="flex justify-between items-start">
          <div className="px-3 py-1 rounded-full text-gray-900 text-sm font-medium bg-[#FEF100]">
            {lesson.topic}
          </div>
          {lesson.duration && (
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center justify-center w-10 h-10 bg-white/90 backdrop-blur-sm rounded-full group-hover:bg-white transition-colors">
            <Icon icon={PlayIcon} size="lg" weight="fill" className="text-neutral-900 ml-1" />
          </div>
        </div>
...
export function TikTokLessons({ title = 'Listen to some friends.', subtitle = 'These winners from last year have some advice for you.', topics = [], maxItems }: TikTokLessonsProps) {
  ...
  return (
    <section className="w-full">
      <div className="mb-4">
        <h2 className="text-xl font-semibold text-neutral-900 leading-none !mb-1">
          {title}
        </h2>
        {subtitle && (
          <p className="text-neutral-600 m-0 leading-none text-sm">
            {subtitle}
          </p>
        )}
      </div>
